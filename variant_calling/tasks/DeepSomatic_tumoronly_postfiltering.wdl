...
task filter_out_germline_variants {
    ...
    command <<<
        mkdir ~{sample}
        bcftools isec -p ~{sample} ~{sample}/~{somatic_VCF_pass_only} ~{sample}/~{germline_VCF_pass_only}
    ...
    output {
        File deepsomatic_only_vcf = "~{sample}/~{sample}_deepsomatic_only.vcf.gz"
        File deepsomatic_only_vcf_idx = "~{sample}/~{sample}_deepsomatic_only.vcf.gz.tbi"
    }
...
task subtract_segdup_regions {
    input {
        ...
        File segdup_bed = /private/groups/patenlab/jimin/data/BED/GIAB/GRCh38_segdups.bed
    ...
    command <<<
        ...
        bedtools subtract -header -a ~{deepsomatic_only_GQ20_DP10_VCF} -b ~{segdup_bed} | bgzip > ~{sample}_deepsomatic_only_GQ20_DP10_segdup.vcf.gz
